...
const initialUISetup = () => {
  ...
  document
    .getElementById("search")
    .addEventListener("keyup", (event) => searchInputHandler(event));
  document
    .getElementById("addTaskInput")
    .addEventListener("keyup", (event) => validateField(event));
...
const addNewTask = () => {
  ...
  document
    .getElementById(id)
    .addEventListener("click", (event) => onRadioClick(event));
};
...
const getTaskTemplate = (value, id) => {
  return `<div class = 'task plainTask' id='${id}'>
             <div class='taskName'>
               <div class = 'radio leftElement'><img width='15' src='src/images/check.png'>
			      </div>
